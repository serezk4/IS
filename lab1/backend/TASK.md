...
private int x; //Максимальное значение поля: 373
private float y; //Максимальное значение поля: 289
}
public class Human {
...
Удалить все объекты, значение поля timezone которого эквивалентно заданному.
Удалить один (любой) объект, значение поля government которого эквивалентно заданному.
Вернуть массив объектов, значение поля name которых содержит заданную подстроку.
Переселить всё население города с id1 в город с id2 (id1 и id2 указываются пользователем)
Переселить 50% жителей столицы в три города с наименьшим населением.

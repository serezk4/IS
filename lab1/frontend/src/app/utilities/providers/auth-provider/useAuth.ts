...
export const useAuth = () => {
    ...
    const logout = useCallback(async (): Promise<void> => {
        // best-effort: даже если запрос упадет, локально выйдем
        try {
    ...
    // Автозагрузка/сброс профиля при смене accessToken
    useEffect(() => {
        if (accessToken) {
            void refreshUser();
        } else {
    ...
    return useMemo(() => ({
        ...
        isAuthenticated,        // надёжный флаг (и токен, и профиль)
        user,                   // BasicUser | null
        login,
        signup,
        logout,
        refreshUser,            // ручное обновление профиля
        setUser,                // опционально: локальный апдейт
        accessToken,
        initialRender

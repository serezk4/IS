...
export function AppHeader() {
    ...
    return (
        <header
            className="border-b sticky top-0 z-50 w-full border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60"
            aria-details='site-header'>
            <MainGutter>
                <div className="container flex h-14 max-w-screen-2xl items-center">
                    ...
                    <span className='flex gap-2 items-center m-2'>Дорохин Сергей, 412934</span>
                            ...
                            <AuthHeaderFx>
                                <div className='flex gap-2'>
                                    <div className='flex items-center space-x-2'>
                                        ...
                                        <span className='text-sm'>[{user?.realm_access.roles.filter(
                                            (role) => role !== 'offline_access' && role !== 'uma_authorization'
                                                && role !== 'default-roles-lockbox'
                                        ).join(', '
                                        )}]</span>
                                    </div>
